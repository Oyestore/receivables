...
import { PaymentPattern } from '../entities/payment-pattern.entity';
 ...
 */
@Injectable()
export class PaymentPatternService {
  private readonly logger = new Logger(PaymentPatternService.name);

  constructor(
    @InjectRepository(PaymentPattern)
    private paymentPatternRepository: Repository<PaymentPattern>,
   ...
   */
  async detectPatterns(buyerId: string, tenantId: string): Promise<PaymentPattern[]> {
  ...
  private async detectAmountPattern(
    ...
    Object.entries(amountRanges).forEach(([amount, payments]) => {
      const paymentArray = payments as PaymentHistory[];
   ...
   */
  private calculateAmountTimelinesCorrelation(ranges: any[]): number {
   ...
   */
  async getActivePatterns(buyerId: string, tenantId: string): Promise<PaymentPattern[]> {
