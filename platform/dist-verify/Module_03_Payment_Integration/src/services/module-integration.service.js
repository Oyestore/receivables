...
exports.ModuleIntegrationService = void 0;
...
const milestone_verification_service_1 = require("./milestone-verification.service");
let ModuleIntegrationService = class ModuleIntegrationService {
    constructor(milestoneService, milestonePaymentService, milestoneEvidenceService, milestoneVerificationService) {
        this.milestoneService = milestoneService;
        this.milestonePaymentService = milestonePaymentService;
        this.milestoneEvidenceService = milestoneEvidenceService;
        this.milestoneVerificationService = milestoneVerificationService;
    }
    /**
     * Configure integration with other modules
     */
    async configureModuleIntegration(config) {
     ...
     */
    async generateMilestoneInvoice(request) {
     ...
     */
    async distributeMilestoneInvoice(request) {
     ...
     */
    async processMilestonePayment(request) {
     ...
     */
    async integrateWithAnalytics(request) {
        const { tenantId, milestoneIds, projectId, reportType, dateRange } = request;
        // Get milestone data for analytics
        let milestones = [];
        if (milestoneIds) {
            milestones = await Promise.all(milestoneIds.map(id => this.milestoneService.findOne(id, tenantId)));
        }
        else if (projectId) {
        ...
        const paymentData = await Promise.all(milestones.map(m => this.milestonePaymentService.findByMilestone(m.id, tenantId))).then(payments => payments.flat());
     ...
     */
    async syncWithIntegratedModules(milestoneId, tenantId) {
        const milestone = await this.milestoneService.findOne(milestoneId, tenantId);
        if (!milestone) {
            throw new Error('Milestone not found');
        }
    ...
    async testModuleConnection(config) {
    ...
    getDefaultPaymentTerms(milestone) {
        return {
    ...
    async callInvoiceGenerationAPI(data) {
    ...
    async callInvoiceDistributionAPI(data) {
    ...
    async callPaymentIntegrationAPI(data) {
    ...
    async syncWithAnalytics(milestone, tenantId) {
    ...
    async syncWithInvoiceModules(milestone, tenantId) {
...
exports.ModuleIntegrationService = ModuleIntegrationService;
exports.ModuleIntegrationService = ModuleIntegrationService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [typeof (_a = typeof milestone_service_1.MilestoneService !== "undefined" && milestone_service_1.MilestoneService) === "function" ? _a : Object, typeof (_b = typeof milestone_payment_service_1.MilestonePaymentService !== "undefined" && milestone_payment_service_1.MilestonePaymentService) === "function" ? _b : Object, typeof (_c = typeof milestone_evidence_service_1.MilestoneEvidenceService !== "undefined" && milestone_evidence_service_1.MilestoneEvidenceService) === "function" ? _c : Object, typeof (_d = typeof milestone_verification_service_1.MilestoneVerificationService !== "undefined" && milestone_verification_service_1.MilestoneVerificationService) === "function" ? _d : Object])
], ModuleIntegrationService);
//# sourceMappingURL=module-integration.service.js.map